# Декораторы и контекстные менеджеры — часть 1
...
Декоратор оборачивает функцию или класс и добавляет новое поведение, не меняя их исходный код.

На практике декоратор обычно является функцией: он принимает другую функцию и возвращает функцию-обёртку.
...
Запись `@decorator` означает: создать функцию, передать её в декоратор и сохранить полученную обёртку под прежним именем.
...
Фабрика декораторов нужна, когда декоратору нужно передать настройки. Это функция, которая принимает настройки и возвращает сам декоратор.
...
`functools.wraps` сохраняет имя, документацию и другие данные исходной функции в функции-обёртке. Без него отладчик и тестовые инструменты будут видеть имя `wrapper` вместо настоящего имени функции.
...
Контекстный менеджер выполняет подготовку перед блоком `with` и обязательную очистку после него — даже если внутри возникло исключение.

Так работают открытие и закрытие файлов, блокировки, соединения и транзакции.
